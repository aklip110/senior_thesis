...
def invertAll(num, paramArray, vert=False):
    ...
    for row in range(len(XYdata)):
        fixedVec, oldVec, paramVec = invert(XYdata, row, paramArray, Xp, Xm, Yp, Ym)
        print(fixedVec)
        print(oldVec)
    ...
    if vert == False:
        np.savetxt(filePath + "data_" + dataName + "/" + "data_" + dataName + "-3D_scan-FIXED.txt", fixedDat)
        np.savetxt(filePath + "data_" + dataName + "/paramDat-3D_scan.txt", paramDat)
