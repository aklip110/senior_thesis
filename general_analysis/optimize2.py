...
def bFunc(row, phi, psi, theta, XYdata, Xp, Xm, Yp, Ym):
    """
    ...
    for i in range(len(XYdata)):
        if (XYdata[i, 0] == T) and (XYdata[i, 1] == R) and (XYdata[i, 9] == onoff):
            bmag += np.sqrt(((XYdata[i, 3])**2 + (XYdata[i, 5])**2 + (XYdata[i, 7])**2))
            bObsX += XYdata[i, 3]
            bObsY += XYdata[i, 5]
            bObsZ += XYdata[i, 7]
            counter += 1
    ...
    #print("b0: ", b0)
    #bObsVec = np.array([XYdata[row, 3], XYdata[row, 5], XYdata[row, 7]])
...
def bFuncSqrs(x, row, XYdata, Xp, Xm, Yp, Ym):
    """
    ...
    for i in ivals:
        #print("deltab: ", bFunc(t, r, phi, a, b, theta)[i])
        LSQ += (bFunc(row, phi, psi, theta, XYdata, Xp, Xm, Yp, Ym)[i])**2
        #print("LSQval: ", LSQ)
    return LSQ
