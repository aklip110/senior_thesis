#z is vertical
#for use with the 4-14_13-51 dataset
#Goal: grab the off data. rotate each row (both the standard rotations and then the corrections (future)). save rotated data
#no averaging.
...
print("Off Data")
#import data ------------------------------
file = "/Users/alexandraklipfel/Desktop/senior_thesis/data_4-14_13-51/data_4-14_13-51-3D_scan.txt"
Alldat = np.loadtxt(file)
print("Data Shape: ", Alldat.shape)
length = Alldat.shape[0]

#import z data
zfile = "/Users/alexandraklipfel/Desktop/senior_thesis/data_4-14_13-51/data_4-14_13-51-vertical_sweep.txt"
...
for i in range(length):
    if (Alldat[i, 9] == 0): #could add vertical filter if wanted
        Offdat = np.vstack([Offdat, Alldat[i,0:13]])
for i in range(Zlength):
    if (zdat[i, 9] == 0): #could add vertical filter if wanted
        ZOffdat = np.vstack([ZOffdat, zdat[i,0:13]])
...
print(trans_row(dat[0, :], plusX, minX, plusY, minY))
